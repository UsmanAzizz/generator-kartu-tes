...
function App() {
  ...
  const downloadTemplate = () => {
    const data = [
      { Nama: "AJENG SETIA RAHAYU", Kelas: "X AKL", No_Pst: "2025/UKK/101" },
      { Nama: "ALYA JESIKA", Kelas: "XII TJKT 2", No_Pst: "2025/UKK/102" }
    ];
  ...
  const handleExcelUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      const mapped = data.map((item, index) => ({
        id: index + 1,
        nama: item.Nama || "NAMA TIDAK ADA",
        kelas: item.Kelas || "-",
        username: item.No_Pst || "-", // Mengisi kolom No. Pst.
      }));
      setStudents(mapped);
    };
